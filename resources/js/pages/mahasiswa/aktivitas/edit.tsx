...
export default function AktivitasEdit({ aktivitas, kategori }: { aktivitas: AktivitasData; kategori: Kategori[] }) {
    const [form, setForm] = useState({
        kategori_id: String(aktivitas.kategori_id),
        nama_kegiatan: aktivitas.nama_kegiatan,
        tahun_kegiatan: aktivitas.tahun_kegiatan,
        peran: aktivitas.peran,
        bukti_link: aktivitas.bukti_link ?? '',
        juara: aktivitas.juara ?? '',
    ...
    function handleSubmit() {
        setProcessing(true);
        router.put(`/mahasiswa/aktivitas/${aktivitas.id}`, form, {
            onSuccess: () => toast.success('Aktivitas berhasil diperbarui.'),
        ...
        <>
            ...
            <div className="space-y-6 p-4 md:p-6">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-semibold">Edit Aktivitas</h1>
                </div>
                    ...
                    <CardContent>
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input id="nama_kegiatan" value={form.nama_kegiatan} onChange={(e) => setForm((p) => ({ ...p, nama_kegiatan: e.target.value }))} />
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input id="tahun_kegiatan" type="number" min={1900} max={2099} value={form.tahun_kegiatan} onChange={(e) => setForm((p) => ({ ...p, tahun_kegiatan: e.target.value }))} />
                                {errors.tahun_kegiatan && <p className="text-xs text-destructive">{errors.tahun_kegiatan}</p>}
                            </div>

                            <div className="grid gap-2">
                                <Label htmlFor="peran">Peran</Label>
                                <Input id="peran" value={form.peran} onChange={(e) => setForm((p) => ({ ...p, peran: e.target.value }))} />
